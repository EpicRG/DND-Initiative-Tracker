...
# Import player names from a .ini file
...
def load_player_names():
    ...
    names = config.get("players", "names", fallback="")
...
def list_character_and_rolls():
    ...
    # Start with custom characters, then add only the checked preset players.
    ...
    # Keep the sorting rule in Backend so the window only handles display.
...
def add_character(name_entry, roll_entry):
    ...
    # Validate every selected player's roll before updating the list.
    selected_players = []
    for player_name, checkbox, player_roll_entry in player_entries:
        if not checkbox.get():
            continue

        try:
            player_roll = Backend.parse_roll(player_name, player_roll_entry.get().strip())
        except ValueError as error:
            print(error)
            return

        selected_players.append((player_name, player_roll))

    # Custom character fields must be either both filled or both empty.
    if bool(name) != bool(roll_text):
        return

    # A custom character is optional; selected preset players are enough.
    custom_character = None
    if name and roll_text:
        try:
            custom_character = (name, Backend.parse_roll(name, roll_text))
        except ValueError as error:
            print(error)
            return

    # Do not refresh the lists when the user submitted no characters at all.
    if not selected_players and custom_character is None:
        return

    if custom_character is not None:
        custom_characters.append(custom_character)
